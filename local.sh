...
if [[ "$(nproc)" -le "12" ]];then
  ...
  if [[ `echo "${PATH}" |grep -c "Windows"` -ge '1' ]]; then
    ...
    PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin make -j$(nproc) |& tee ${HOME_PATH}/build_logo/build.log 2>&1
     ...
     make -j$(nproc) |& tee ${HOME_PATH}/build_logo/build.log 2>&1
  fi
else
  ECHOGG "您的CPU线程超过或等于32线程，强制使用32线程进行编译固件,请耐心等候..."
  sleep 5
  if [[ `echo "${PATH}" |grep -c "Windows"` -ge '1' ]]; then
    ECHOY "WSL临时路径编译中,请耐心等候..."
    PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin make -j32 |& tee ${HOME_PATH}/build_logo/build.log 2>&1
  else
     make -j32 |& tee ${HOME_PATH}/build_logo/build.log 2>&1
  fi
fi
